...
def task(data, template, model_0, sae_0, hook_point_0, model_1, sae_1, hook_point_1):
...
def process_task(task_name, config):
    ...
    # get model
    model = HookedTransformer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", hf_model = local_model, device=config["device"])
    ...
    intersection_mean, intersection_variance, value_changes_mean, value_changes_variance = task(task_data, template, model_0, sae_0, hook_point_0, model, sae_1, hook_point_1)

    # general data 
    intersection_mean_general, intersection_variance_general, value_changes_mean_general, value_changes_variance_general = task(general_data, template, model_0, sae_0, hook_point_0, model, sae_1, hook_point_1)
...
final_df.to_csv("outputs/mistral_output.csv", index=False)
