...
def process_task(task_name, config, topk_range):
    ...
    # get model
    model_1 = HookedTransformer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", hf_model = local_model, device=device[1])
...
def plot_results(results, topk_range):
    ...
    plt.savefig("outputs/figures/mistral_mean_change.pdf")
...
def main():
    ...
    with open("outputs/raw/results_mistral.pkl", "wb") as f:
        pickle.dump(results, f)
    
    # with open("outputs/raw_results_mistral.json", "w") as f:
