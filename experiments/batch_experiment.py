...
def process_task(task_name, config):
    ...
    # get model
    model = HookedTransformer.from_pretrained("Qwen/Qwen1.5-0.5B-Chat", hf_model = local_model, device=config["device"])
...
final_df.to_csv("outputs/qwen_output.csv", index=False)
