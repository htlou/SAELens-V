...
def l0_test(sae, hook_language_model, token_dataset,description, batch_size=8):
    ...
    with torch.no_grad():
        ...
        # 汇总所有批次的 L0 范数
        l0_all = torch.cat(l0_list)
        # 输出所有批次的平均l0带上描述
        print(
            f"平均 L0 for {description}: {l0_all.mean().item()}")
...
def main():
    ...
    token_dataset_llava = load_and_tokenize_dataset(
        ...
        split="train",
        tokenizer=hook_llava_model.tokenizer,
        max_length=sae_V_llava.cfg.context_size,
        add_bos_token=sae_V_llava.cfg.prepend_bos
    )
